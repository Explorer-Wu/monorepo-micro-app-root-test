...
export class HttpAxios {
	...
	constructor(options: ExtraTypeOpts, config?: AxiosRequestConfig) {
		console.log('HttpAxios_constructor:', options, config);
		// axios实例配置
		this.axiosConfig = {
			...this.axiosConfig,
			baseURL: options.envBaseUrl,
			...config,
			/**
			 *  `transformRequest` 允许在向服务器发送前，修改请求数据
			 * 只能用于 'PUT', 'POST' 和 'PATCH' 这几个请求方法
			 * 数组中最后一个函数必须返回一个string、 plain object、 ArrayBuffer、 ArrayBufferView、URLSearchParams
			 * 或 - 浏览器专属：FormData, File, Blob
			 * 或 - Node 专属： Stream
			 */
			transformRequest: [
				...
				(data: any, headers: any): any => {
					...
					// dealReqHeaders(headers, data);
					return transformReqHeaderData(headers, data);
				},
			],
			transformResponse: [
	...
	private interceptorsRequest() {
		...
		this.axiosInstance.interceptors.request.use(
			...
			(
				...
				if (config.authtoken) {
					let getTokenKey = getUserToken(
						this.otherOpts?.envTokenKey,
						this.otherOpts?.envRefreshKey,
					); // localStorage.getItem('user_token');
					getTokenKey.accessToken &&
						config.headers &&
						(config.headers[
							this.otherOpts?.envTokenKey === 'ai_auth' || !this.otherOpts?.envTokenKey
								? 'Authorization'
								: this.otherOpts?.envTokenKey
		...
		this.axiosInstance.interceptors.response.use(
			...
			(error: NewAxiosError<ResDataTypeMode, any>) => {
				...
				return Promise.reject(
					new Error(message, {
						cause: !$quietMsg && `${message}${headers?.traceId ?? ''}`,
					}),
				);
			},
		);
	...
	// 公共请求方法 ajax
	public ajax<T = any, R = NewAxiosResponse<ResDataTypeMode<T>, any>, D = any>(
		opts: ReqOptsConfig<D>,
	): Promise<R> {
	...
	public get<T = any, R = NewAxiosResponse<ResDataTypeMode<T>, any>, D = any>(
		opts: ReqOptsConfig<D>,
	): Promise<R> {
		return this.ajax({ method: 'GET', ...opts });
	}
	public post<T = any, R = NewAxiosResponse<ResDataTypeMode<T>, any>, D = any>(
		opts: ReqOptsConfig<D>,
	): Promise<R> {
		return this.ajax({ method: 'POST', ...opts });
	}
	public put<T = any, R = NewAxiosResponse<ResDataTypeMode<T>, any>, D = any>(
		opts: ReqOptsConfig<D>,
	): Promise<R> {
		return this.ajax({ method: 'PUT', ...opts });
	}
	public delete<T = any, R = NewAxiosResponse<ResDataTypeMode<T>, any>, D = any>(
		opts: ReqOptsConfig<D>,
	): Promise<R> {
		return this.ajax({ method: 'DELETE', ...opts });
	}
	public upload<T = string, R = NewAxiosResponse<ResDataTypeMode<T>, any>>(
		fileItem: UploadFileItemType,
		opts?: UploadReqConfig,
	): Promise<R> | null {
